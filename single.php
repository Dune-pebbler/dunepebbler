...
<main>
  ...
  if (!empty($thumb)) {
    ?>
    <section class="video-container half-height <?php if($post_type == 'story_collection' || $post_type == 'post'){ echo 'higher-item'; } ?>">
          ...
          <?php if (have_rows('afbeeldingen')): ?>
            <div class='col-md-3 offset-md-1 col-12'>
              ...
              while (have_rows('afbeeldingen')): the_row();
                ...
                ?>
                <figure class='img'>
                  <img src='<?php echo $afbeelding['url']; ?>' loading="lazy" alt="<?php echo $afbeelding['alt']; ?>" style='max-width: 100%;'/>
                </figure>
              <?php endwhile; ?>
            </div>
          <?php endif; ?>
  ...
  <?php if ($post->post_type == 'vacature'): ?>
    <section class="vacature-form">
      ...
      <button class="btn btn-primary" id="send-form">
        <span><?= __('Send now!', 'dunepebbler'); ?> <span class="contact-send-arrow"><i class="fa fa-angle-right" aria-hidden="true"></i></span></span>
      ...
      <div class="row">
        <div class="col-xl-10 offset-xl-1 col-12">
          <div class="date-author">
            ...
            <div class="author">
              <span><?= __('Geschreven door:', 'dunepebbler'); ?> </span>
              <figure>
                <img src="<?= $get_author_gravatar; ?>" alt="" loading="lazy">
              </figure>

              <?php the_author(); ?>

              <span class="author-date"><?php __('Gepubliceerd op:', 'dunepebbler'); ?> <?= get_the_date('d F Y'); ?></span>
            </div>
          </div>
        </div>
      </div>
